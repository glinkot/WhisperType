...
// Modified by MF to allow progress bar
public static class WhisperGgmlDownloader
{
    ...
    public static async Task<(Stream, long)> GetGgmlModelAsync(GgmlType type, QuantizationType quantization = QuantizationType.NoQuantization, CancellationToken cancellationToken = default)
    {
        ...
        {
            contentLength = Convert.ToInt64(response.Content.Headers.First(h => h.Key == "Content-Length").Value.First());
        }
        else
        {
            throw new Exception("Content-Length not present");
        }
    ...
    public static string GetModelName(GgmlType type)
    {
        return type switch
        {
